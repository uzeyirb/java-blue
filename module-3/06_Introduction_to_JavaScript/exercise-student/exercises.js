...
function frontAgain(value) {
	let first2 = value.slice(0, 2);
	let last2 = value.slice(value.length - 2, value.length);
	if (first2 == last2) {
		return true;
	} else {
		return false;
...
*/
function cigarParty(value, weekend) {
	if (value >= 40 && value <= 60 && weekend == false) {
		return true;
	}
	if (value >= 40 && weekend == true) {
		return true;
	} else {
		return false;
	}
...
function filterBigNumbers(myArray) {
	let myNewArray = [];
	for (let i = 0; i < myArray.length; i++) {
		if (myArray[i] >= 100) {
			myNewArray.push(myArray[i]);
...
*/
function filterMultiplesOfX(myArray, myValue){
	let myNewArray = [];
	for (let i = 0; i < myArray.length; i++) {
