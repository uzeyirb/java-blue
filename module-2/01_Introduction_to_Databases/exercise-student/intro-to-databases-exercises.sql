...
FROM country
WHERE continent ILIKE 'O%' AND lifeexpectancy IS NOT NULL;
...
SELECT name, lifeexpectancy
FROM country
WHERE continent ILIKE 'O%' AND lifeexpectancy IS NULL;
...
SELECT name, gnp, population, (gnp / population) AS per_capita_GNP from country Where continent = 'Europe';
