...
--this causes a type error can cast any colum (or aggregate result) to a different datatype
...
SELECT MIN(surfacearea), MAX(surface), AVG(surfacearea), SUM(surfacearea), COUNT(*)
FROM country
...
SELECT MIN(population), MAX(population) FROM city GROUP BY countrycode;